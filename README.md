...
평가용으로 비낚시성 기사 200건을 무작위 추출했고, RAG 검색 코퍼스로는 학습용 기사 40,106건을 사용했습니다.
...
### 설문 기반 인간 평가 (기사 50건, 참여자 7명, 후보 4개)

자동 평가 대상 200건 중 50건을 무작위 추출해 설문으로 진행했습니다.
설문에서는 사람 작성 제목을 제외한 4개 모델만 비교했으며,
'가장 읽고 싶은 제목'(단일 선택)과 '본문과 관련이 없다고 생각되는 제목'(다중 선택)을 함께 물었습니다.

| 모델 | 선호도 | 관련 없음 |
|---|---:|---:|
| **Gemini-2.0-Flash (RAG)** | **33.4%** | 14.6% |
| Gemini-2.0-Flash (Direct) | 24.3% | 17.7% |
| GPT-4o (RAG) | 22.3% | **9.2%** |
| GPT-4o (Direct) | 20.0% | 9.4% |

두 평가 모두에서 선호도 순위가 같았고, RAG 적용 모델이 같은 계열의 Direct 모델보다
**선호도는 높고 비정합성은 낮았습니다.**

다만 흥미성과 정합성의 1위는 서로 다릅니다. 선호도 1위인 Gemini 계열은 비정합성이 14.6~17.7%로
GPT-4o 계열(9.2~9.4%)보다 높습니다. Gemini 는 표현의 다양성과 창의성을,
GPT-4o 는 기사에 충실한 제목을 만드는 경향으로 해석되며, 목적에 따라 모델 선택이 달라질 수 있습니다.
